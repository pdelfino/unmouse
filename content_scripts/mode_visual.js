...
class Movement {
  ...
  // Run a movement. This is the core movement method.
  runMovement(...args) {
    ...
    if ((granularity === vimword) && (direction === forward)) {
      while (this.nextCharacterIsWordCharacter()) {
        if (this.extendByOneCharacter(forward) === 0) return;
      }
      while (this.getNextForwardCharacter() && !this.nextCharacterIsWordCharacter()) {
        if (this.extendByOneCharacter(forward) === 0) return;
      }
      if (this.alterMethod === "move") this.selection.collapseToEnd();
      return;
    }
  ...
  getDirection() {
    for (const direction of [forward, backward]) {
      ...
      if (change) {
        ...
        if (change > 0) return direction;
        else return this.opposite[direction];
      }
    }
    return forward;
  }
...
class VisualMode extends KeyHandlerMode {
  init(options) {
    ...
    // Build the key mapping structure required by KeyHandlerMode. Handles single-char keys,
    // two-char key sequences (like "gg"), and angle-bracket keys (like "<left>").
    const keyMapping = {};
    ...
    super.init(Object.assign(options, {
      ...
      indicator: options.indicator != null ? options.indicator : "Unmouse: Visual",
    ...
    // Establish or use the initial selection.
    if (this.name !== "caret") {
  ...
  yank(args) {
    ...
    HUD.show(`Copied ${this.yankedText.length} character${plural}: \"${message}\".`, 2500);
...
class VisualLineMode extends VisualMode {
  init(options) {
    if (options == null) options = {};
    super.init(Object.assign(options, { name: "visual/line", indicator: "Unmouse: Line" }));
...
class CaretMode extends VisualMode {
  init(options) {
    if (options == null) options = {};
    super.init(
      Object.assign(options, { name: "caret", indicator: "Unmouse: Caret", alterMethod: "move" }),
    );
...
globalThis.CaretMode = CaretMode;
