...
const Utils = {
  ...
  isFirefox() {
    if (!this._browserInfoLoaded) return false; // Default to not Firefox if info hasn't loaded.
    return this._isFirefox;
  },

  firefoxVersion() {
    if (!this._browserInfoLoaded) return null;
  ...
  monitorChromeSessionStorage(key, setter) {
    try {
      chrome.storage.session.get(key, (obj) => {
        if (chrome.runtime.lastError) return; // Session storage not ready yet.
        if (obj[key] != null) setter(obj[key]);
      });
    } catch (e) {
      // Session storage may not be accessible yet; ignore.
    }
